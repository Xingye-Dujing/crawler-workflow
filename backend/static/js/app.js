...
const I18n = {
    ...
    dict: {
        en: {
            ...
            'settings.profileOffHint': 'This platform is known to refuse a throwaway browser: its session cookie rotates, or a replayed one is rejected within minutes. Turn on the persistent browser profile in Settings. A COPY of a live login counts as a second device to the site — what it refuses is often the file in your hand, not the account.',
            ...
            'settings.commentFetchHint': 'These comments are fetched from inside one loaded page, so a headless run opens no window (only a platform that has to scroll opens one); every part_size comments are written as a part file into the export directory.',
            ...
            'runsMgr.tagHeadlessMixed': 'headless → window',
        ...
        zh: {
            ...
            'settings.author': '作者',
            'settings.profileOffHint': '这个平台已知会拒绝一次性浏览器：会话 Cookie 会换票，重放的旧票几分钟内就被拒。请在「设置」里打开持久浏览器 Profile。把登录态拷进另一个浏览器，在站点看来就是第二台设备——它拒的往往是你手里那份文件，不是这个账号。',
            ...
            'settings.commentFetchHint': '此平台的评论在页面内取数，无头运行时不会弹窗；每 part_size 条评论写入一个分片文件到导出目录。',
            ...
            'runsMgr.tagHeadlessMixed': '无头→窗口',
