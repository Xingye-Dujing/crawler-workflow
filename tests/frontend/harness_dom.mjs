...
/**
 * Read a JSON fixture the pytest driver wrote and passed as an argument.
 *
 * The crawl matrix is generated by the backend (`crawl_capabilities.as_dict()`),
 * so a harness must be handed that exact payload rather than a copy checked into
 * tests/frontend — a copy is how the panel and the executor would drift back
 * apart the moment a platform changes.
 */
...
export function baseSandbox() {
    ...
    const stored = {};
    /* Kept as its own object rather than read back off `sandbox`: a harness
       builds its world with `{...baseSandbox()}`, which copies `fetch` but not
       the object it closes over, so a route written to the copy would never be
       seen. Shared by reference, seeding a payload works from either side. */
    const routes = {};
    const sandbox = {
        console,
        JSON,
        ...
        fetch: (url) => {
            /* The Data Source panel is rendered from the server's crawl matrix, so
               a harness that opens one has to answer that endpoint with something
               real: `__routes` maps a URL fragment to a payload. Anything else
               keeps the old `{ok:true}` reply, which the product reads as "the
               server answered nothing usable" — the honest failure path, not a
               throw inside its own catch. */
            const hit = Object.keys(routes).filter((frag) => String(url).indexOf(frag) >= 0)[0];
            const payload = hit ? routes[hit] : { ok: true };
            return Promise.resolve({ ok: true, json: () => Promise.resolve(payload) });
        },
        /* The same object the `fetch` above reads, handed out so a harness can
           seed `/api/capabilities` (or any other endpoint) before driving code
           that calls it. */
        __routes: routes,
        alert: () => {},
        confirm: () => true,
