...
def _refused_when_blank() -> list:
    """The steps that need a column, a new name or a rename pair to do anything.

    Read off the gate's own table rather than pasted: this file used to keep its own
    tuple of these names, and ``tests/unit/test_data_analysis.py`` fails any file that
    does — a list of operation names is a copy that can disagree with the registry
    while still passing every test that uses it.
    """
    from services.data_analysis import STEP_PARAMS

    return sorted(
        name for name, spec in STEP_PARAMS.items() if spec.get('one') or spec.get('nonblank') or spec.get('mapping')
    )


def _inert_when_blank() -> list:
    """The steps allowed to do nothing when their form is blank — 留空=全部 and friends.

    ``join_tables`` is left out: the node executor, not the normalizer, is the only
    caller that can supply its right-hand table (pinned by its own test below).
    """
    from services.data_analysis import STEP_PARAMS

    refused = set(_refused_when_blank())
    return sorted(name for name in STEP_PARAMS if name not in refused and name != 'join_tables')
...
class TestNormalizedKwargs:
    ...
    def test_a_cleared_field_is_not_an_absent_one_for_five_fields(self, app_module, op, cleared):
        """CURRENT behaviour, pinned on purpose. Five fields are read with
        ...
        a cleared ``op`` is refused by name and a cleared ``value`` fills with the
        empty string. The fields that go through ``or`` instead (``how``, ``method``,
        ``n``, ``frac``, ``seed``, ``bins``, ``bin_labels``) read the two states the
        same way — that inconsistency is what a workflow file has to guess about.

        ``ascending`` used to be on this list. A switch has no third position to
        preserve, so it is now read by ``as_bool``, for which a cleared box is exactly
        the untouched one: the direction the panel still shows.
        """
        blank = BLANK_FORM[op]
        cleared_form = {
            'value': '',
    ...
    def test_a_cleared_switch_is_the_untouched_switch(self, app_module):
        """The one parameter deliberately moved out of the list above."""
        assert _normalize(app_module, 'sort_rows', {'column': '序号', 'ascending': ''}) == _normalize(
            app_module, 'sort_rows', {'column': '序号'}
        )
    ...
    @pytest.mark.parametrize('op', _inert_when_blank())
    def test_an_untouched_form_never_destroys_a_row(self, app_module, op):
        """The safety half of the defaults: an unconfigured step that can mean
        "every column" may be a no-op, it may not be a filter. ``join_tables`` is
        excluded because the node executor — not the normalizer — is the only caller
        that can supply its right-hand table.
        """
        import pandas as pd
    ...
    @pytest.mark.parametrize('op', _refused_when_blank())
    def test_an_untouched_form_that_names_nothing_is_refused_by_that_name(self, app_module, op):
        """The other half of the same guarantee, and the one the old test could not
        state: a step missing the thing it operates on is not "harmless", it is a
        green light on work that did not happen. The refusal names the parameter, so
        the panel knows which box to go back to.
        """
        import pandas as pd
        ...
        frame = pd.DataFrame(CLEAN_TABLE)
        params = _normalize(app_module, op, {})
        with pytest.raises(UnknownOperationError) as excinfo:
            DataAnalysisService.run_pipeline(frame, [{'op': op, 'params': params}])
        assert op in str(excinfo.value), str(excinfo.value)

    def test_the_two_halves_cover_every_step(self):
        """Nothing may fall between them, and the split is not a preference: it is
        read off the gate, so a step whose spec changes moves between the halves on
        its own.
        """
        from services.data_analysis import STEP_PARAMS

        registry = set(_registry())
        assert set(_inert_when_blank()) | set(_refused_when_blank()) | {'join_tables'} == registry
        assert set(STEP_PARAMS) == registry, 'the gate and this test disagree about which steps exist'
        # The refused half is what the panel can actually leave blank on purpose.
        assert set(_refused_when_blank()) <= set(BLANK_FORM), 'a refused step has no blank form to test'
...
class TestJunkInTheStringFields:
    ...
    names, whatever arrives. That the names exist is not the parser's business —
    the pipeline gate refuses a list naming a column this table does not have, in
    :meth:`test_a_name_that_is_not_a_column_is_refused_instead_of_widening_the_step`.
    """
    ...
    def test_a_name_that_is_not_a_column_is_refused_instead_of_widening_the_step(self, client, paste):
        """The service used to drop names it could not find, an empty selection meant
        "no subset", and 任一为空 then read *every* column — so a typo in 列名 deleted
        more rows than the one it named. The pipeline gate refuses the name now, which
        is the answer a person can act on.
        """
        ...
        assert named['row_count'] == 2, named
        typo = client.post(
            '/api/analysis/run',
            json={'dataset_id': dataset_id, 'steps': [{'op': 'drop_null', 'params': {'columns': ['nope']}}]},
        )
        assert typo.status_code == 400, typo.get_json()
        error = typo.get_json()['error']
        assert 'nope' in error and 'drop_null' in error, error

    def test_an_empty_column_list_still_means_every_column(self, client, paste):
        """The half of the rule the refusal above must not swallow: 留空=全部 is what
        the panel says and what saved pipelines do, so a blank list is not a mistake.
        """
        records = [{'a': 1, 'b': None}, {'a': 2, 'b': 3}]
        dataset_id = paste(records, name='blank-columns.csv')
        for columns in ([], '', '   ', ',,,', None):
            body = client.post(
                '/api/analysis/run',
                json={'dataset_id': dataset_id, 'steps': [{'op': 'drop_null', 'params': {'columns': columns}}]},
            ).get_json()
            assert body['row_count'] == 1, (columns, body)
...
def _run_analysis_chain(client, app_module, paste, operation, params, records=None, steps=None):
    """``upload → 分析 node``, answered with the node's own status and stored rows.
    ...
    The status is returned as well as the rows, because half of this file's
    contract is a node that must NOT settle done: a step this table cannot carry
    out has to fail with its reason, and asserting only "no rows" would also be
    satisfied by an operation that legitimately found nothing to keep.
    """
...
def _run_analysis_node(client, app_module, paste, operation, params, records=None, steps=None):
    """The same chain, for the cases that expect the node to succeed."""
    status, rows = _run_analysis_chain(client, app_module, paste, operation, params, records=records, steps=steps)
...
class TestFilterOperatorsThroughTheApi:
    ...
    def test_a_column_that_is_not_in_the_table_is_refused_for_every_operator(self, app_module, client, paste):
        """For every one of the twelve names: a filter cannot decide anything about a
        column this table does not have, and passing the rows through untouched used to
        settle the node DONE — so an export shipped *unfiltered* data under a green
        check, and a rename earlier in the pipeline turned a filter into a silent
        pass-through. Refusing names the column instead.
        """
        ...
        for op in sorted({row[1] for row in FILTER_SWEEP.values()}):
            ...
            assert '不存在' in error and 'filter_rows' in error, (op, error)
    ...
    def test_an_operator_the_service_does_not_know_is_refused_by_name(self, app_module, client, paste):
        ...
        opinion), so the refusal has to come from somewhere down the line — for every
        one of the twelve names, each asked for with a suffix that makes it unknown. A
        ...
        The gate in ``run_pipeline`` answers first now, so the sentence is the
        catalogued one and lists the names the parameter does accept; the operator's own
        refusal stays as the second line of defence for a direct caller.
        """
        ...
        for op in sorted({row[1] for row in FILTER_SWEEP.values()}):
            ...
            assert response.status_code == 400, (op, response.get_json())
            error = response.get_json()['error']
            assert unknown in error, (op, error)
            assert 'eq' in error, f'{op} was refused without naming what it accepts: {error}'
...
@pytest.mark.serial
class TestStepEnumsRefusedByName:
    """Every select-shaped parameter of a cleaning step, on the list or refused by name.

    The four spellings a step used to accept for one meaning are the point: an unknown
    ``convert_type`` dtype produced **text** (the ``else`` branch of a chain that never
    said so), an unknown ``fill_null`` method produced a literal fill (so
    ``method='mean'`` wrote the word 'mean' into every empty cell), and an unknown
    ``drop_null`` ``how`` died inside pandas with a stack trace no console line could
    attribute. ``/api/analysis/run`` and the analysis node both pass the same gate.
    """

    @pytest.mark.parametrize(
        ('op', 'key', 'allowed'),
        [
            ('drop_null', 'how', ['any', 'all']),
            ('fill_null', 'method', ['ffill', 'pad', 'bfill', 'backfill']),
            ('convert_type', 'dtype', ['str', 'int', 'float', 'bool', 'datetime']),
            ('join_tables', 'how', ['left', 'right', 'inner', 'outer', 'cross']),
            (
                'filter_rows',
                'op',
                [
    ...
    def test_every_name_the_step_accepts_still_reaches_the_step(self, client, paste, op, key, allowed):
        """The refusal must not have cost a step any of its real answers.

        Sent as a step list, not through the normalizer: the gate is what decides here,
        and ``_normalize_analysis_params`` owns only the shape of a value (pinned by
        :class:`TestNormalizedKwargs`). ``join_tables`` still needs a right-hand table no
        HTTP caller can supply, so reaching *that* complaint is the proof its ``how`` was
        accepted.
        """
        dataset_id = paste(CLEAN_TABLE, name=f'enum-{op}-{key}.csv')
        extra = {
            'drop_null': {'columns': ['名称']},
            'fill_null': {'columns': ['名称'], 'value': 'x'},
            'convert_type': {'column': '名称'},
            'join_tables': {'left_on': '名称', 'right_on': '名称'},
            # A numeric column: four of the twelve names compare with float(), so a
            # text column would answer 400 for a reason that is not this test's subject.
            'filter_rows': {'column': '序号', 'value': '3'},
        }[op]
        for name in allowed:
            response = client.post(
                '/api/analysis/run', json={'dataset_id': dataset_id, 'steps': _step(op, {**extra, key: name})}
            )
            error = (response.get_json() or {}).get('error', '')
            assert response.status_code == 200 or 'other_df' in error, (op, key, name, response.status_code, error)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ('drop_null', 'how', 'both'),
            ('drop_null', 'how', 'Any'),
            ('fill_null', 'method', 'mean'),
            ('convert_type', 'dtype', 'number'),
            ('convert_type', 'dtype', 'Int64'),
            ('join_tables', 'how', 'full'),
        ],
    )
    def test_a_name_the_step_does_not_have_is_answered_with_the_list_it_does(self, client, paste, op, key, wrong):
        """Case is not folded here: ``'Any'`` and ``'Int64'`` are names this step does not
        have, and guessing them into ``any``/``int`` is the same silent substitution one
        spelling shorter down would have to guess too.
        """
        dataset_id = paste(CLEAN_TABLE, name=f'enum-bad-{op}.csv')
        extra = {
            'drop_null': {'columns': ['名称']},
            'fill_null': {'columns': ['名称'], 'value': 'x'},
            'convert_type': {'column': '名称'},
        ...
        response = client.post(
            '/api/analysis/run', json={'dataset_id': dataset_id, 'steps': _step(op, {**extra, key: wrong})}
        )
        assert response.status_code == 400, response.get_json()
        error = response.get_json()['error']
        assert wrong in error and key in error and op in error, error

    def test_a_padded_option_is_the_option_it_spells(self, client, paste):
        """Stripping happens before the check *and* before the operation, so a name that
        passes is a name that runs — a gate that forgave padding the operator would still
        choke on is a gate that lies.
        """
        dataset_id = paste(CLEAN_TABLE, name='enum-padded.csv')
        body = client.post(
            '/api/analysis/run',
            json={'dataset_id': dataset_id, 'steps': _step('drop_null', {'columns': [' 名称 '], 'how': '  all  '})},
        ).get_json()
        # 全部为空 on this table: no row has every column empty, so nothing is dropped —
        # which is also the proof the padded subset resolved instead of widening.
        assert body['row_count'] == len(CLEAN_TABLE), body
        assert body['report'][0]['params'] == {'columns': ['名称'], 'how': 'all'}, body['report']

    def test_a_blank_option_is_the_step_s_own_default_and_is_not_refused(self, client, paste):
        """Deleting the text in a dropdown is not a wrong answer; it is no answer, and the
        step's declared default is what the panel showed.
        """
        dataset_id = paste(CLEAN_TABLE, name='enum-blank.csv')
        for op, extra, key in (
            ('drop_null', {'columns': ['名称']}, 'how'),
            ('fill_null', {'columns': ['名称'], 'value': 'x'}, 'method'),
            ('convert_type', {'column': '名称'}, 'dtype'),
        ):
            for blank in ('', '   ', None):
                response = client.post(
                    '/api/analysis/run', json={'dataset_id': dataset_id, 'steps': _step(op, {**extra, key: blank})}
                )
                assert response.status_code == 200, (op, blank, response.get_json())


@pytest.mark.serial
class TestABooleanAndATextMeanTheSameThing:
    """``ascending`` is a switch, and a switch has two spellings on disk.

    The browser's checkbox writes the strings ``'true'``/``'false'``; a workflow file,
    an API caller or any later serialization writes a real JSON boolean. The normalizer
    used to compare against one of the two (``params.get('ascending', 'true') ==
    'true'``), which read the *other* one as "descending" — the direction the user chose
    inverted, silently, and the run still settled done. Both readings are now
    ``utils.helpers.as_bool``, which knows the two grammars, both interface languages,
    case and padding, and answers the declared default for a value that states nothing.

    ``_to_bool`` in this package is NOT the same helper: it is the reading of a
    *scraped cell*, where anything not falsey is on. Applied to a parameter it would
    turn a cleared box into "descending", which is the opposite of the figure the panel
    shows in that box.
    """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ('yes', True),
            ('off', False),
            # A value that is neither spelling answers the declared default (ascending).
            ('', True),
            (None, True),
            ('abc', True),
            (['true'], True),
    ...
    def test_either_grammar_selects_the_same_direction(self, app_module, sent, ascending):
        out = _normalize(app_module, 'sort_rows', {'column': '序号', 'ascending': sent})
        assert out['ascending'] is ascending, sent
        # An absent field is the documented default, the only other way up.
        assert _normalize(app_module, 'sort_rows', {'column': '序号'})['ascending'] is True

    def test_a_boolean_true_sorts_the_table_up(self, client, app_module, paste):
        """The end-to-end half of the grid, from the workflow file a caller writes:
        the boolean used to come back in the opposite order to the one the box claims.
        """
    ...
    def test_the_word_true_sorts_it_the_same_way(self, client, app_module, paste):
        boolean = _run_analysis_node(client, app_module, paste, 'sort_rows', {'column': '序号', 'ascending': True})
        text = _run_analysis_node(client, app_module, paste, 'sort_rows', {'column': '序号', 'ascending': 'true'})
        assert [row['序号'] for row in boolean] == [row['序号'] for row in text]

    def test_a_cleared_box_is_the_default_direction(self, client, app_module, paste):
        """Deleting what was in the box is not the same click as unticking it: the
        stored text becomes ``''``, which states nothing, and 升序 stays checked in the
        panel. The old comparison answered that with a descending table.
        """
        cleared = _run_analysis_node(client, app_module, paste, 'sort_rows', {'column': '序号', 'ascending': ''})
        assert [row['序号'] for row in cleared][:4] == [1, 3, 4, 5]
...
class TestRenameWithoutANewName:
    ...
    def test_a_rename_without_a_new_name_fails_the_node(self, client, app_module, paste):
        """The panel shows two boxes; filling only the first is a half-finished thought.

        It used to rename the column to the empty string and settle DONE, so every
        later reader of that table — the preview, the chart's field dropdown, the export
        header — looked at a column called ``""`` (or at nothing, because a blank key is
        what the browser uses for "no column chosen"). Now the step is refused and the
        column keeps its name.
        """
        status, rows = _run_analysis_chain(client, app_module, paste, 'rename_columns', {'rename_from': '城市'})
        assert status.get('status') == 'failed', status
        error = status.get('error') or ''
        assert '城市' in error and 'rename' in error, error
        assert rows == [], 'a refused step must not hand out a table with a nameless column'

    def test_a_rename_naming_no_such_column_fails_the_node(self, client, app_module, paste):
        status, rows = _run_analysis_chain(
            client, app_module, paste, 'rename_columns', {'rename_from': 'nope', 'rename_to': '新'}
        )
        assert status.get('status') == 'failed', status
        assert 'nope' in (status.get('error') or '')
        assert rows == []

    def test_a_rename_that_names_both_columns_renames_it(self, client, app_module, paste):
        """The refusal is about the missing half, not about renaming.

        ``FILTER_TABLE`` has no 城市 column, so the pair this file's own fixtures do
        have is the one that has to keep working.
        """
        rows = _run_analysis_node(
            client,
            app_module,
            paste,
            'rename_columns',
            {'rename_from': '名称', 'rename_to': 'renamed'},
            records=[{'名称': 'a', 'n': 1}, {'名称': 'b', 'n': 2}],
        )
        assert {'renamed' in row for row in rows} == {True}, rows
        assert '名称' not in rows[0], rows
...
@pytest.mark.serial
class TestBothStepPathsReadTheSameParameters:
    """A step list and a flat single-operation node now mean the same thing.

    ``_normalize_analysis_params`` used to be called from exactly one place: the branch
    that turns a node's *flat* settings into one step. A node carrying a step list (the
    shape ``/api/analysis/run`` accepts, and the shape a workflow file can hold) sent its
    params straight to ``run_pipeline`` as kwargs, so ``columns: '名称, 城市'`` was not a
    list of two names but an iterable of seven characters — a step that quietly did
    nothing, invisible from a UI whose two forms look identical. The reading moved into
    ``run_pipeline``, which both doors pass through, so this class pins the equality
    that the asymmetry used to break.
    """

    def test_a_panel_shaped_step_now_selects_the_columns_it_names(self, client, paste):
        dataset_id = paste(FILTER_TABLE, name='raw-steps.csv')
        steps = [{'op': 'select_columns', 'params': {'columns': '名称, 城市'}}]
        body = client.post('/api/analysis/run', json={'dataset_id': dataset_id, 'steps': steps}).get_json()
        assert set(body['preview'][0]) == {'名称', '城市'}, body['preview']
        assert body['report'][0]['params'] == {'columns': ['名称', '城市']}, body['report']

    def test_the_same_words_as_a_single_operation_select_two_columns(self, client, app_module, paste):
        """The two spellings of one intent, side by side."""
        dataset_id = paste(FILTER_TABLE, name='normalized-step.csv')
        params = _normalize(app_module, 'select_columns', {'columns': '名称, 城市'})
        body = client.post(
            '/api/analysis/run', json={'dataset_id': dataset_id, 'steps': _step('select_columns', params)}
        ).get_json()
        assert set(body['preview'][0]) == {'名称', '城市'}

    def test_a_node_with_a_step_list_is_the_same_path(self, client, app_module, paste):
        """Both node doors — ``steps`` and the flat form — answer the same table."""
        ...
        assert [set(row) for row in raw] == [set(row) for row in normalized]
        assert set(raw[0]) == {'名称', '城市'}

    def test_a_padded_column_name_still_resolves(self, client, paste):
        """Padding comes off a name, the way ``Field.value_from`` takes it off a crawl
        field: ``' 名称 '`` is the same column, not a typo to be dropped.
        """
        dataset_id = paste(FILTER_TABLE, name='padded-steps.csv')
        body = client.post(
            '/api/analysis/run',
            json={'dataset_id': dataset_id, 'steps': [{'op': 'select_columns', 'params': {'columns': [' 名称 ']}}]},
        ).get_json()
        assert set(body['preview'][0]) == {'名称'}, body['preview']
...
class TestStepOrderIsTheAnswer:
    ...
    def test_the_same_steps_in_another_order_are_refused_not_silently_unfiltered(self, client, paste):
        """Reversing the first two steps asks the filter to read a column the rename
        has not produced yet.

        It used to keep all five rows and answer 200, which is the same lie in a
        different clothes: the pipeline reported the filter's result while returning
        the unfiltered table. The order still decides — that is the point — but a
        step whose column does not exist where it runs is now an error naming it.
        """
        ...
        assert response.status_code == 400, response.get_json()
        error = response.get_json()['error']
        assert 'city' in error and 'filter_rows' in error, error

    def test_the_forward_order_still_filters_after_the_rename(self, client, paste):
        """The other direction, unchanged: the same two steps, wired so the column is
        there by the time it is read.
        """
        dataset_id = paste(FILTER_TABLE, name='ordered-steps.csv')
        steps = _step('rename_columns', {'mapping': {'城市': 'city'}}) + _step(
            'filter_rows', {'column': 'city', 'op': 'contains', 'value': '京'}
        )
        body = client.post('/api/analysis/run', json={'dataset_id': dataset_id, 'steps': steps}).get_json()
        assert body['row_count'] == 2, body['preview']
