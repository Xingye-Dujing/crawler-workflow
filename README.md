...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/哔哩哔哩/抖音/YouTube/X（推特） 按关键词采集，YouTube 与 X 另可选「某作者的作品」（YouTube 填 @handle/频道链接/UC… ID，X 填 @handle 或主页链接），微信按粘贴的推文链接采集正文；「采集内容」切到**评论**即变为评论采集器。可选项完全由后端 `/api/capabilities` 的采集矩阵生成，新增平台或模式不必再改前端 | 是，需平台+关键词/作者（或评论/推文链接） |
