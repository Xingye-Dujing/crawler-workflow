...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/哔哩哔哩/抖音按关键词采集，微信按粘贴的推文链接采集正文；「采集内容」切到**评论**即变为评论采集器。可选项完全由后端 `/api/capabilities` 的采集矩阵生成，新增平台或模式不必再改前端 | 是，需平台+关键词（或评论/推文链接） |
...
### 自动化测试（pytest，快速层约 2185 用例，全层 2277）
