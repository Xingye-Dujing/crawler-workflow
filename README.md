...
### 数据采集
- **多平台爬虫**：支持知乎、微博、小红书、微信公众号、哔哩哔哩、抖音、YouTube 的数据采集
  （X（推特）与 Instagram 已可保存 Cookie，采集实现待接入）
...
- **评论采集**：数据源节点把「采集内容」切到**评论**，粘贴所选平台的文章链接（一行一个）即可批量抓评论；
  知乎/微博/小红书/哔哩哔哩/抖音/YouTube 各自只认本平台链接——面板样例、画布预检、引擎校验、抓取时过滤四层一致拒收外站链接，
...
- **Cookie 管理**：持久化登录状态，避免重复扫码（知乎/微博/小红书/哔哩哔哩/抖音，加上 **X（推特）/
  ...
  按关键词搜索因实测不可验证已移除）。
  其中 **X（推特）与 Instagram 目前只能保存登录 Cookie，还不能作为数据源采集**：画布的数据源节点不会
  列出它们，即便手写进工作流也会被按节点名明确拒绝，而不是回一张空表（YouTube 已实装，见下条）。
...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/哔哩哔哩/抖音/YouTube 按关键词采集，YouTube 另可选「某作者的作品」（填 @handle/频道链接/UC… ID），微信按粘贴的推文链接采集正文；「采集内容」切到**评论**即变为评论采集器。可选项完全由后端 `/api/capabilities` 的采集矩阵生成，新增平台或模式不必再改前端 | 是，需平台+关键词/作者（或评论/推文链接） |
...
### 自动化测试（pytest，快速层约 2323 用例，全层 2424）
...
| `live_site`（marker） | **全平台真实爬取 × 双浏览器模式**：知乎/微博/小红书/哔哩哔哩/YouTube 各自的搜索抓取、微信的推文正文抓取，都分别以 无头 与 可见窗口 两种模式真跑真断言；抖音因无头必被挡在验证码中间页，故以**可见窗口**真跑搜索＋逐视频详情，并单独断言「无头必须报可执行错误而不是交 0 行」；YouTube 反过来钉两种模式**结果一致**（它走页内 JSON），并真跑「某作者的作品」与「按频道链接再进一次」；评论节点六平台真测（微博/小红书无头、哔哩哔哩无头游标翻页、知乎与抖音强制可见、YouTube 游标翻页；抖音以「评论数≥30 的视频至少收到 30 条」证明容器滚动真的在喂列表、(作者,内容) 不重复；YouTube 以「三个最热视频至少有一个翻页成功（>20 条）」证明游标没停在第一页）与**真浏览器 Cookie 中途失效→续跑**用例；知乎无头风控零结果按设计语义处理（明确无结果页→可执行报错，未触发请求的空壳→合法 0 行）；无对应 Cookie 自动跳过 | Chrome + 已存 Cookie |
