...
- **微信 Cookie 的两个用途（实测结论）**：同一份 Cookie 文件服务两件事，且入口不同——
  **按关键词搜索**需要 `mp.weixin.qq.com` **公众号后台**登录态（搜狗微信搜索一访问即被
  `antispider` 验证码拦截，机器无法自行通过）；**抓取推文评论**需要把 PC 微信里「复制链接」
  得到的链接（携带 `pass_ticket`）填进登录入口。普通网页链接在浏览器里服务端只渲染空的留言容器
  （三篇真实文章实测：`wx_getext_config` 为 null、`discuss_list` 0 条，换 MicroMessenger UA 亦然），
  因此这种情况会被判为"未被授权查看留言"并给出可操作提示，而不是谎报"该文无留言"
