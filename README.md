...
- **Cookie 管理**：持久化登录状态，避免重复扫码（知乎/微博/小红书/哔哩哔哩/抖音五个平台；
  微信**不在其中**——抓推文正文本来就不需要登录，唯一需要后台登录的按关键词搜索因实测不可验证已移除）；
...
### 自动化测试（pytest，快速层约 2006 用例，全层 2086）
...
| `tests/integration` | LLM 传输边界（OpenRouter 一律 mock，绝不真实请求）、行运行器（断点/熔断/取消）、真 Chrome 解析 `file://` 夹具、微博/知乎假驱动登录墙判定、**哔哩哔哩假驱动翻页契约**（首页不带 `page=`、广告/课程卡不耗请求、`-404` 跳过而非停摆、非 `-404` 的拒绝在零结果时raise 可执行错误、游标按页记录并可续页）、**抖音假驱动契约**（搜索框送词＋按钮提交、按钮被遮罩时回落回车、验证码标题→可执行错误而非空表、行数只带自解释的四个计数、**断言行里没有播放数列**、已开过的视频不重复付费、评论滚动去重）、**真浏览器 UI 回归：自启服务器后逐面板 × 中英双语实测「容器不得溢出自身盒子、页面级不出水平滚动条」**（声明可滚的容器单独放行，但其中被截断的单元格必须带省略号——静默裁掉等于让用户读不到数据；表单控件自身的文字滚动不算截断），窄窗口下浮动面板不得跑到视口外，节点框宽度上限与标题省略号、标记里没有内联事件、委托出去的编辑/删除按钮在真实点击下仍作用于自己那个节点、双击改名走对话框、草稿恢复保住节点 id 且后续新建不撞号、撤销重做保住 id 与用户起的名字、`escapeHtml` 在真 DOM 里把日志文本挡在元素之外（该层不写服务器：应用没有 data 目录覆盖开关，跑一次流程就会往真实 `data/` 与 `logs/` 里留东西） | 夹具层需 Chrome |
