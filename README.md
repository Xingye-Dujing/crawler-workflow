...
### 自动化测试（pytest，快速层约 1799 用例，全层 1841）
...
| `tests/api` | Flask test_client：覆盖 50+ 端点，含「上传→清洗→导出」完整执行链路、Cookie 登录状态机、**Cookie 用途接口与验证任务**（无 Cookie 直接拒、越界链接拒、探测崩溃判 error 不挂面板、登录按钮不得应答验证、验证与登录共用单飞锁）、**Cookie 中途过期→节点判失败→换 Cookie 续跑不重复**的完整模拟、**十类节点逐一执行验证（分词/图表/导出格式/续跑收养）**、**导出产物三端点**（列表分页与合计分离、下载按名解析、`../` 与绝对路径与子目录一律 404、删除越界名不生效、畸形请求 400）、**停止（空闲/运行中）→记录判中断且数据保全、线程强杀保护主线程、卡死浏览器按 PID 定向回收**、设置面板布尔开关、API 加固（畸形请求→400） | 无（外部调用全 mock） |
