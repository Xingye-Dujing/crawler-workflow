...
### 数据采集
- **多平台爬虫**：支持知乎、微博、小红书、微信公众号、哔哩哔哩的数据采集
  （抖音目前只支持保存登录 Cookie，采集实现待其真实 DOM/接口分析后落地，运行时会按节点名明确拒收而不是给空表）
...
- **评论采集**：数据源节点把「采集内容」切到**评论**，粘贴所选平台的文章链接（一行一个）即可批量抓评论；
  知乎/微博/小红书/哔哩哔哩各自只认本平台链接——面板样例、画布预检、引擎校验、抓取时过滤四层一致拒收外站链接，
...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/微信/哔哩哔哩采集数据；「采集内容」切到**评论**即变为评论采集器 | 是，需平台+关键词（或评论链接） |
...
| `tests/integration` | LLM 传输边界（OpenRouter 一律 mock，绝不真实请求）、行运行器（断点/熔断/取消）、真 Chrome 解析 `file://` 夹具、微博/知乎假驱动登录墙判定、**哔哩哔哩假驱动翻页契约**（首页不带 `page=`、广告/课程卡不耗请求、`-404` 跳过而非停摆、非 `-404` 的拒绝在零结果时raise 可执行错误、游标按页记录并可续页）、**真浏览器 UI 回归：自启服务器后实测设置/AI 子菜单中英文双语均无水平滚动条，看板/历史弹窗外点关闭与内部点击保留** | 夹具层需 Chrome |
| `live_ollama`（marker） | 真调本地 Ollama 的活模型用例（服务不可达自动跳过） | Ollama |
| `live_site`（marker） | **全平台真实爬取 × 双浏览器模式**：知乎/微博/小红书/微信/哔哩哔哩各自的搜索或文章/视频抓取都分别以 无头 与 可见窗口 两种模式真跑真断言，另加评论节点四平台真测（微博/小红书/哔哩哔哩无头、知乎强制可见；哔哩哔哩用 40 条上限证明游标真的翻页、rpid 不重复、子回复挂回父评论）与**真浏览器 Cookie 中途失效→续跑**用例；知乎无头风控零结果按设计语义处理（明确无结果页→可执行报错，未触发请求的空壳→合法 0 行）；无对应 Cookie 自动跳过 | Chrome + 已存 Cookie |
