...
- **评论采集**：数据源节点把「采集内容」切到**评论**，粘贴所选平台的文章链接（一行一个）即可批量抓评论；
  ...
  **微信例外（实测）**：文章页在浏览器里拿不到留言凭证，留言接口固定回“请在微信客户端打开链接”，
  因此微信评论节点会明确报「被服务端拒绝（无权查看）」而不是 0 行成功——详见下文 Cookie 一节
...
- **微信 Cookie 的两个用途（实测结论）**：同一份 Cookie 文件服务两件事，入口却不同——
  **按关键词搜索**只有 `mp.weixin.qq.com` **公众号后台**登录态这一条路（搜狗微信搜索一访问即被
  `antispider` 验证码拦截，机器无法自行通过）。
  **推文留言在纯 Chrome 里拿不到**，而且常见猜测已被逐一排除：客户端「复制链接」现在给的是干净
  `/s/xxx`（不含 `pass_ticket`）；查客户端自己的浏览记录显示，它打开的也是**同一个裸链接**；
  换 `MicroMessenger`/`XWEB` UA、加 `scene`/`devicetype`/`version` 等客户端风格参数、以及
  **重放客户端 Cookie 库里 `pass_ticket`/`appmsg_token` 等 11 条 `mp.weixin.qq.com` Cookie**
  （三种组合分别实测）都改变不了结果——留言接口始终回一段 2034 字节的验证页「请在微信客户端打开链接」。
  放行判定因此不在 URL、不在 Cookie、也不在 UA，而在微信客户端网络栈的设备级绑定上。
  本项目把这种情况实现为**明确拦截**：评论节点报「被服务端拒绝（本会话无权查看留言）」，
  绝不用 0 行冒充"这篇文章没有评论"（`tests/live_site/test_live_wechat.py::TestCommentRefusal` 钉住该不变量）
