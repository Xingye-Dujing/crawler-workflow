...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/哔哩哔哩/抖音/YouTube/X（推特） 按关键词采集，知乎、YouTube 与 X 另可选「某作者的作品」（知乎填主页链接或 /people/ 后面的 id，YouTube 填 @handle/频道链接/UC… ID，X 填 @handle 或主页链接），微信按粘贴的推文链接采集正文；「采集内容」切到**评论**即变为评论采集器。可选项完全由后端 `/api/capabilities` 的采集矩阵生成，新增平台或模式不必再改前端 | 是，需平台+关键词/作者（或评论/推文链接） |
...
| `live_site`（marker） | **全平台真实爬取 × 双浏览器模式**：知乎/微博/小红书/哔哩哔哩/YouTube 各自的搜索抓取、微信的推文正文抓取，都分别以 无头 与 可见窗口 两种模式真跑真断言；抖音因无头必被挡在验证码中间页，故以**可见窗口**真跑搜索＋逐视频详情，并单独断言「无头必须报可执行错误而不是交 0 行」；YouTube 反过来钉两种模式**结果一致**（它走页内 JSON），并真跑「某作者的作品」与「按频道链接再进一次」；X **只以可见窗口真跑**（搜索 22 行以上，证明虚拟列表确实被 id 窗口推着走；作者模式取 @NASA 时间线；无头那一路断言的是「绝不安静地交 0 行」——要么拒、要么给真数据），并真读一条**本次搜到的**推文的回复；评论节点七平台真测（微博/小红书无头、哔哩哔哩无头游标翻页、知乎与抖音强制可见、YouTube 游标翻页、X 强制可见并按 id 窗口滚过第一屏；抖音以「评论数≥30 的视频至少收到 30 条」证明容器滚动真的在喂列表、(作者,内容) 不重复；YouTube 以「三个最热视频至少有一个翻页成功（>20 条）」证明游标没停在第一页）与**真浏览器 Cookie 中途失效→续跑**用例；知乎无头风控零结果按设计语义处理（明确无结果页→可执行报错，未触发请求的空壳→合法 0 行）；无对应 Cookie 自动跳过 | Chrome + 已存 Cookie |
