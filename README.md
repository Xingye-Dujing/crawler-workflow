...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/微信/哔哩哔哩/抖音采集数据；「采集内容」切到**评论**即变为评论采集器 | 是，需平台+关键词（或评论链接） |
...
| `tests/integration` | LLM 传输边界（OpenRouter 一律 mock，绝不真实请求）、行运行器（断点/熔断/取消）、真 Chrome 解析 `file://` 夹具、微博/知乎假驱动登录墙判定、**哔哩哔哩假驱动翻页契约**（首页不带 `page=`、广告/课程卡不耗请求、`-404` 跳过而非停摆、非 `-404` 的拒绝在零结果时raise 可执行错误、游标按页记录并可续页）、**抖音假驱动契约**（搜索框送词＋按钮提交、按钮被遮罩时回落回车、验证码标题→可执行错误而非空表、行数只带自解释的四个计数、**断言行里没有播放数列**、已开过的视频不重复付费、评论滚动去重）、**真浏览器 UI 回归：自启服务器后实测设置/AI 子菜单中英文双语均无水平滚动条，看板/历史弹窗外点关闭与内部点击保留** | 夹具层需 Chrome |
| `live_ollama`（marker） | 真调本地 Ollama 的活模型用例（服务不可达自动跳过） | Ollama |
| `live_site`（marker） | **全平台真实爬取 × 双浏览器模式**：知乎/微博/小红书/微信/哔哩哔哩各自的搜索或文章/视频抓取都分别以 无头 与 可见窗口 两种模式真跑真断言；抖音因无头必被挡在验证码中间页，故以**可见窗口**真跑搜索＋逐视频详情，并单独断言「无头必须报可执行错误而不是交 0 行」；评论节点五平台真测（微博/小红书无头、哔哩哔哩无头游标翻页、知乎与抖音强制可见；抖音以「评论数≥30 的视频至少收到 30 条」证明容器滚动真的在喂列表、(作者,内容) 不重复）与**真浏览器 Cookie 中途失效→续跑**用例；知乎无头风控零结果按设计语义处理（明确无结果页→可执行报错，未触发请求的空壳→合法 0 行）；无对应 Cookie 自动跳过 | Chrome + 已存 Cookie |
