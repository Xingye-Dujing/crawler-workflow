...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/哔哩哔哩/抖音/YouTube/X（推特） 按关键词采集，知乎、YouTube、X（推特）与哔哩哔哩 另可选「某作者的作品」（知乎填主页链接或 /people/ 后面的 id，YouTube 填 @handle/频道链接/UC… ID，X 填 @handle 或主页链接，哔哩哔哩填 space.bilibili.com/<UID> 链接或数字 UID），微信按粘贴的推文链接采集正文；「采集内容」切到**评论**即变为评论采集器。可选项完全由后端 `/api/capabilities` 的采集矩阵生成，新增平台或模式不必再改前端 | 是，需平台+关键词/作者（或评论/推文链接） |
...
再走他自己的空间列表，断言每一行都属于这一个 UP、播放数仍是接口给的整数、游标里记着走的是谁的空间）；评论节点七平台真测（微博/小红书无头、哔哩哔哩无头游标翻页、知乎与抖音强制可见、YouTube 游标翻页、X 强制可见并按 id 窗口滚过第一屏；抖音以「评论数≥30 的视频至少收到 30 条」证明容器滚动真的在喂列表、(作者,内容) 不重复；YouTube 以「三个最热视频至少有一个翻页成功（>20 条）」证明游标没停在第一页）与**真浏览器 Cookie 中途失效→续跑**用例；知乎无头风控零结果按设计语义处理（明确无结果页→可执行报错，未触发请求的空壳→合法 0 行）；无对应 Cookie 自动跳过 | Chrome + 已存 Cookie |
