...
| Data Source（数据源） | SRC | 从知乎/微博/小红书/哔哩哔哩/抖音/YouTube/X（推特） 按关键词采集，知乎、YouTube、X（推特）、哔哩哔哩 与抖音 另可选「某作者的作品」（知乎填主页链接或 /people/ 后面的 id，YouTube 填 @handle/频道链接/UC… ID，X 填 @handle 或主页链接，哔哩哔哩填 space.bilibili.com/<UID> 链接或数字 UID，抖音填 douyin.com/user/… 链接或那串 sec_uid），微信按粘贴的推文链接采集正文；「采集内容」切到**评论**即变为评论采集器。可选项完全由后端 `/api/capabilities` 的采集矩阵生成，新增平台或模式不必再改前端 | 是，需平台+关键词/作者（或评论/推文链接） |
...
再走他自己的空间列表，断言每一行都属于这一个 UP、播放数仍是接口给的整数、游标里记着走的是谁的空间）；抖音 也真跑「某作者的作品」（`sec_uid` 当场从**本次搜到并打开过**的视频页上取 `a[href*="/user/"]`，
